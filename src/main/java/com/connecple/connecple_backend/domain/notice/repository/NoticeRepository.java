...
public interface NoticeRepository extends JpaRepository<NoticeManagement, Long> {
  ...
  // 클라이언트용: 페이징 지원
  Page<NoticeManagement> findAllByIsActiveTrueAndIsDeletedFalse(Pageable pageable);

  Page<NoticeManagement> findAllByCategoryInAndIsActiveTrueAndIsDeletedFalse(List<String> categories,
      Pageable pageable);
